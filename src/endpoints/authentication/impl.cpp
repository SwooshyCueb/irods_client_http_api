...
namespace irods::http::handler
{
	...
	auto authentication(session_pointer_type _sess_ptr, request_type& _req) -> void
	{
		...
		if (_req.method() == boost::beast::http::verb::get) {
			...
			// Check to see if we have the code query
			if (did_except) {
				...
				boost::asio::post(thread_pool, [fn = __func__, _sess_ptr, _req = std::move(_req)] {
					...
					const auto auth_endpoint{
						irods::http::globals::oidc_endpoint_configuration()->at("authorization_endpoint").get_ref<const std::string&>()};

					const auto yep{fmt::format(
						"{}?client_id={}&response_type=code&scope=openid&redirect_uri={}&state={}",
						auth_endpoint,
						irods::http::globals::oidc_configuration()->at("client_id").get_ref<const std::string&>(),
						get_encoded_redirect_uri(),
						"placeholder")};
				...
				auto& thread_pool{*irods::http::globals::thread_pool_bg};
				boost::asio::post(thread_pool, [fn = __func__, _sess_ptr, _req = std::move(_req), url=std::move(url)] {
					...
					// Build body
					auto thing_thang{fmt::format(
						"client_id={}&grant_type=authorization_code&code={}&redirect_uri={}",
						irods::http::globals::oidc_configuration()->at("client_id").get_ref<const std::string&>(),
						code_iter->second,
						get_encoded_redirect_uri())};

					// Hit endpoint, get res
					nlohmann::json res_item{hit_token_endpoint(thing_thang)};
				...
				if (std::string_view::npos == pos) {
					...
					// Network context
					{
						...
						beast::http::request<beast::http::string_body> req{
							beast::http::verb::post, path, version_number};
						req.set(beast::http::field::host, host);
						req.set(beast::http::field::user_agent, BOOST_BEAST_VERSION_STRING);
						req.set(
						...
						string_builder << "client_id="
									   << irods::http::globals::oidc_configuration()->at("client_id").get<const std::string>()
									   << '&' << "grant_type=password&"
									   << "scope=openid&"
						...
						// JSONize response
						nlohmann::json res_item{hit_token_endpoint(send_to)};
